...
public class EBaiduMapOverlayMgr implements OnMarkerClickListener {
	...
	private void addMarkOp(final EBaiduMapMarkerOverlayOptions markerOverlayOptions, Bitmap bitmap) {
		...
		if (type != null) {// 如果type不为空
			if (type.equals(EBaiduMapUtils.MAP_BUBBLE_TYPE_SHANGHAI_BEIER)) {// 如果是上海贝尔bubbleType
				String width = markerOverlayOptions.getWidth();
				String bgColor = markerOverlayOptions.getBgColor();
				String titleLeftTop = markerOverlayOptions.getTitle1();
				String titleRightTop = markerOverlayOptions.getTitle2();
				String titleBottom = markerOverlayOptions.getTitle3();
				markerOverlay.setBubbleViewDataShanghaiBeier(width, bgColor, titleLeftTop, titleRightTop, titleBottom,
						yOffset, isUse);// 设置为上海贝尔定制气泡
			}
		} else if (markerOverlayOptions.getBubbleTitle() != null) {
