...
public class EBaiduMapUtils {
	...
	public static EBaiduMapMarkerOverlayOptions getMarkerOverlayOpitonsWithJSON(String jsonStr) {
		...
		try {
			...
			String type = null;// 气泡类型，用来区分气泡风格
			...
			if (makerJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TYPE_SHANGHAI_BEIER)) {
				type = makerJsonObject.getString(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TYPE_SHANGHAI_BEIER);
			}
			...
			markerOverlayOptions.setType(type);
			...
			String width = null;// 气泡宽度
			String bgColor = null;// 背景颜色
			String title1 = null;// 左上角文本
			...
			if (makerJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_BUBBLE)) {
				...
				// 上海贝尔定制版，增加字段 by_waka
				if (bubbleJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_WIDTH_SHANGHAI_BEIER)) {
					width = bubbleJsonObject.getString(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_WIDTH_SHANGHAI_BEIER);
				}
				if (bubbleJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_BG_COLOR_SHANGHAI_BEIER)) {
					bgColor = bubbleJsonObject.getString(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_BG_COLOR_SHANGHAI_BEIER);
				}
				if (bubbleJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TITLE_LEFT_TOP_SHANGHAI_BEIER)) {
					title1 = bubbleJsonObject
							.getString(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TITLE_LEFT_TOP_SHANGHAI_BEIER);
				}
				if (bubbleJsonObject.has(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TITLE_RIGHT_TOP_SHANGHAI_BEIER)) {
					title2 = bubbleJsonObject
							.getString(EBaiduMapUtils.MAP_PARAMS_JSON_KEY_TITLE_RIGHT_TOP_SHANGHAI_BEIER);
				}
			...
			// 上海贝尔定制版，增加字段 by_waka
			markerOverlayOptions.setWidth(width);
			markerOverlayOptions.setBgColor(bgColor);
